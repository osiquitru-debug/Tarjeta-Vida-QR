...
# Fondo Menta para Registro y Consulta, Menta muy claro para Inicio
...
st.markdown(f"""
    ...
    /* TARJETA DE EVOLUCIÓN (Sincronizada con Sheet) */
    .evo-card {{
        background-color: #ffffff; padding: 15px; border-radius: 10px;
        border: 1px solid #e2e8f0; border-left: 5px solid #b7e4c7;
...
def cargar_datos():
    try:
        ...
        # Normalizar nombres de columnas a MAYÚSCULAS para evitar errores de tipeo
...
with st.sidebar:
    ...
    if st.button("🏠 Inicio", use_container_width=True): 
        st.session_state.menu = "Inicio"; st.rerun()
    if st.button("📝 Registrar Paciente", use_container_width=True): 
        st.session_state.menu = "Registrar"; st.rerun()
    if st.button("🔍 Consulta / Evolución", use_container_width=True): 
        st.session_state.menu = "Consulta"; st.rerun()
...
if st.session_state.menu == "Inicio":
    ...
    st.write("Soporte Clínico - Guadalupe, Huila.")
    ...
    with st.form("f_registro", clear_on_submit=True):
        ...
        c_nom = st.text_input("Nombre de Referencia"); c_tel = st.text_input("Teléfono de Referencia")
    ...
    if id_buscado and df_p is not None:
        ...
        if not paciente.empty:
            ...
            <div class="medical-card">
                ...
                <p><b>ID:</b> {p.get('DOCUMENTO')} | <b>RH:</b> {p.get('RH')} | <b>EDAD:</b> {p.get('EDAD')}</p>
            ...
            st.subheader("📋 Historial (Sincronizado con Sheet)")
            if not h_p.empty:
                for _, f in h_p.iterrows():
                    st.markdown(f"""
                    <div class="evo-card">
                        <small>📅 {f.get('MARCA TEMPORAL')}</small><br>
                        <p style='margin: 5px 0;'><b>3. MOTIVO DE LA CONSULTA:</b> {f.get('3. MOTIVO DE LA CONSULTA')}</p>
                        <p style='margin: 5px 0;'><b>2. VALORACIÓN:</b> {f.get('2. VALORACIÓN')}</p>
                        ...
                        <p style='margin: 5px 0;'><b>💊 TRATAMIENTO (7/8/9):</b> {f.get('8. MEDICAMENTOS')}</p>
                        <p style='margin: 5px 0; font-size:0.85em; color: gray;'><b>Notas:</b> {f.get('9. EPICRISIS O NOTAS ADICIONALES')}</p>
                    </div>""", unsafe_allow_html=True)
