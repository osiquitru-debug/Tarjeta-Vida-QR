...
st.markdown(f"""
    ...
    /* Estilo de inputs: Fondo blanco y letra negra */
...
if st.session_state.menu == "Inicio":
    ...
    if id_buscado and df_p is not None:
        ...
        if not paciente.empty:
            ...
            pdf.set_font("Arial", 'B', 12); pdf.cell(0, 10, "INFORMACION DEL PACIENTE", 1, 1, 'L', 1)
            pdf.set_font("Arial", '', 10)
            pdf.cell(0, 7, f"Nombre: {p.get('NOMBRE')}", 0, 1)
            pdf.cell(0, 7, f"ID: {p.get('DOCUMENTO')} | RH: {p.get('RH')} | Edad: {p.get('EDAD')}", 0, 1)
            pdf.multi_cell(0, 7, f"Alertas Medicas: {p.get('CONDICIONES ESPECIALES (ALERGIAS, ENFERMEDADES DE BASE)')}")
            pdf.cell(0, 7, f"Contacto Emergencia: {p.get('NOMBRE CONTACTO EMERGENCIA')} - {p.get('TELEFONO CONTACTO EMERGENCIA')}", 0, 1)
            pdf.ln(5)
            ...
            if not h_p.empty:
                ...
                for _, f in h_p.iterrows():
                    ...
                    pdf.cell(0, 5, f"MEDIDAS: Talla: {f.get('4. TALLA')} cm | Peso: {f.get('5. PESO')} kg | Tension: {f.get('6. PRESIÓN ARTERIAL')}", 0, 1)
            ...
            st.download_button("📥 Descargar Reporte Completo (PDF)", pdf.output(dest='S').encode('latin-1'), f"HC_{id_buscado}.pdf")

            # Formulario Evolución
            with st.expander("➕ REGISTRAR NUEVA EVOLUCIÓN"):
                with st.form("f_evo", clear_on_submit=True):
                    c1, c2 = st.columns(2)
                    with c1: v_val = st.text_area("Valoración"); v_mot = st.text_area("Motivo"); v_tal = st.text_input("Talla (cm)")
                    with c2: v_pes = st.text_input("Peso (kg)"); v_pre = st.text_input("Presión"); v_med = st.text_area("Tratamiento"); v_epi = st.text_area("Notas/Epicrisis")
                    if st.form_submit_button("GUARDAR EVOLUCIÓN"):
                        requests.post("https://docs.google.com/forms/d/e/1FAIpQLSeCCQLkQZbbGw_WJPWzYOhZrm6aOgmTQjDsFRD_y4wV6rB8VA/formResponse", 
                                      data={"entry.2019369477": id_buscado, "entry.1088523869": v_val, "entry.611862537": v_mot, "entry.1275746503": v_tal, "entry.949747647": v_pes, "entry.2091389798": v_pre, "entry.2016051626": v_med, "entry.616774918": v_epi})
