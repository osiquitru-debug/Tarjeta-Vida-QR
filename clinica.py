...
# --- 1. CONFIGURACIÓN Y ESTÉTICA (Casillas blancas y títulos centrados) ---
...
st.markdown("""
    ...
    </Limpia el ID de espacios, decimales .0 y caracteres especiales."""
...
# --- 2. CARGA DE DATOS CON NORMALIZACIÓN (Solución al error de búsqueda) ---
URL_CSV = "[https://docs.google.com/spreadsheets/d/18Ohfwj5TkaoRf3oPFpPxpPYhHTpccfLpG5r30MXEvC0/gviz/tq?tqx=out:csv](https://docs.google.com/spreadsheets/d/18Ohfwj5TkaoRf3oPFpPxpPYhHTpccfLpG5r30MXEvC0/gviz/tq?tqx=out:csv)"

def normalizar_id(valor):
    """Elimina .0, espacios y convierte a texto puro para que el buscador no falle."""
...
def cargar_tablas():
    try:
        # Cargamos como texto puro (dtype=str)
        p = pd.read_csv(f"{URL_CSV}&sheet=pacientes", dtype=str).fillna("")
        h = pd.read_csv(f"{URL_CSV}&sheet=historial", dtype=str).fillna("")
        
        # Creamos una columna oculta para la búsqueda infalible
        # El documento suele estar en la columna B (índice 1)
...
# --- 3. NAVEGACIÓN ORIGINAL POR BOTONES ---
...
if st.session_state.menu == "Inicio":
    ...
    st.info("Utilice el menú lateral para gestionar la información de los pacientes.")
    ...
    with st.form("f_reg", clear_on_submit=True):
        ...
        if st.form_submit_button("GUARDAR PACIENTE"):
            ...
            requests.post("[https://docs.google.com/forms/d/e/1FAIpQLSfH5wFiZ57m530cMju3wOnI1m1AynsK3uAINDTvnvMYkiFLZg/formResponse](https://docs.google.com/forms/d/e/1FAIpQLSfH5wFiZ57m530cMju3wOnI1m1AynsK3uAINDTvnvMYkiFLZg/formResponse)", data=payload)
            st.success("✅ Regist<rado en la base de datos."); st.cache_data.clear()
    ...
    if id_limpio and df_p is not None:
        ...
        if not p_match.empty:
            p = p_match.iloc[0]
            # TARJETA MÉDICA (Datos completos usando posiciones iloc)
            st.markdown(f"""
            ...
            with st.expander("✍️ REGISTRAR NUEVA EVOLUCIÓN"):
                with st.form("f_evo", clear_on_submit=True):
                    ...
                    if st.form_submit_button("💾 GUARDAR"):
                        ...
                        requests.post("[https://docs.google.com/forms/d/e/1FAIpQLSeCCQLkQZbbGw_WJPWzYOhZrm6aOgmTQjDsFRD_y4wV6rB8VA/formResponse](https://docs.google.com/forms/d/e/1FAIpQLSeCCQLkQZbbGw_WJPWzYOhZrm6aOgmTQjDsFRD_y4wV6rB8VA/formResponse)", data=h_pay)
            ...
            if not h_p.empty:
                ...
                pdf = FPDF()
                pdf.add_page()
                pdf.set_```font("Arial", 'B', 14); pdf.cell(0, 10, f"HC: {p.iloc[2]}", ln=True)
            ...
            st.error("❌ Paciente no encontrado. Verifique que el número coincida con el Sheet.")
