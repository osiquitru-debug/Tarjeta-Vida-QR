...
# ─────────────────────────────────────────────
# 4. GENERADOR DE CARNET CR80 (85.6 × 54 mm)
# ─────────────────────────────────────────────
def generar_carnet(p, alertas, nom_emer, tel_emer):
    """
    Retorna bytes de un PDF de 2 páginas:
      Página 1 → Cara FRONTAL
      Página 2 → Cara POSTERIOR
    Tamaño exacto CR80: 85.6 × 54 mm
    """
    W = 85.6 * mm
    H = 54.0 * mm
    ...
    edad    = str(p.get("EDAD",      "No registra"))
    celular = str(p.get("CELULAR",   "No registra"))
    ...
    p_wave.curveTo(W*0.25, franja_h + 4*mm,
                   W*0.55, franja_h - 3*mm,
                   W*0.78, franja_h + 2*mm)
    p_wave.curveTo(W*0.88, franja_h + 4*mm,
                   W*0.95, franja_h + 1*mm,
                   W,      franja_h)
    p_wave.lineTo(W, 0); p_wave.lineTo(0, 0)
    p_wave.close()
    c.drawPath(p_wave, fill=1, stroke=0)

    # ── Íconos y textos de la franja ─────────
    icon_data = [
        ("🩺", "INFORMACIÓN\nMÉDICA",      W*0.08),
        ("💊", "MEDICAMENTOS",              W*0.30),
        ("⚠️", "ALERGIAS",                 W*0.52),
    ...
    if _log:
        try:
            c.drawImage(_log, 1.5*mm, franja_h + 3*mm,
    ...
    # ── Caja QR (esquina superior derecha) ───
    qr_size = 20 * mm
    qr_x    = W - qr_size - 2*mm
    qr_y    = H - qr_size - 2.5*mm

    # Marco teal redondeado
    c.setFillColor(colors.HexColor("#00897b"))
    c.roundRect(qr_x - 1.5*mm, qr_y - 5*mm,
                qr_size + 3*mm, qr_size + 6.5*mm, 2*mm, fill=1, stroke=0)
    # QR
    try:
        c.drawImage(_qr, qr_x, qr_y, width=qr_size, height=qr_size)
    except:
        pass
    # Etiqueta bajo QR
    c.setFillColor(colors.white)
    ...
    if _log:
        try:
            c.drawImage(_log, 1.5*mm, H - 18*mm,
    ...
    if id_buscado and df_p is not None:
        ...
        if not paciente.empty:
            ...
            with col_carnet:
                ...
                st.download_button(
                    "🪪 Carnet VidaQR (imprimible)",
                    carnet_bytes,
                    f"Carnet_VidaQR_{id_buscado}.pdf",
                    mime="application/pdf")

            st.info("💡 El carnet tiene 2 páginas: cara frontal y cara posterior. "
                    "Imprime a tamaño **85.6 × 54 mm** (tarjeta de crédito) para un resultado perfecto.", icon="🖨️")
