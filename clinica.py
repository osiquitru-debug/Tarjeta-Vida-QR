...
st.markdown(f"""
    ...
    .grid-medidas {{ 
        display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 10px; margin: 12px 0; padding: 8px 0;
        border-top: 1px solid #f1f5f9; border-bottom: 1px solid #f1f5f9;
    }}
    div.stButton > button {{
        background-color: #84dcc6 !important; color: #000000 !important; border-radius: 10px !important; font-weight: bold !important;
    }}
...
if st.session_state.menu == "Inicio":
    ...
    with st.form("f_registro"):
        c1, c2 = st.columns(2)
        with c1:
            nombre = st.text_input("Nombre Completo"); tipo_doc = st.selectbox("Tipo Doc", ["CC", "TI", "CE", "RC"]); n_doc = st.text_input("Número de Documento")
        with c2:
            edad = st.text_input("Edad"); eps = st.text_input("EPS"); rh = st.selectbox("RH", ["O+", "O-", "A+", "A-", "B+", "B-", "AB+", "AB-"])
    ...
    if id_buscado and df_p is not None:
        ...
        if not paciente.empty:
            ...
            # --- LÓGICA DE PDF INTEGRAL ---
            class PDF(FPDF):
                def header(self):
                    self.image(LOGO_URL, 10, 8, 33)
                    self.set_font('Arial', 'B', 15)
                    self.cell(80)
                    self.cell(30, 10, 'Tarjeta Vida QR', 0, 0, 'C')
                    self.ln(5); self.cell(80); self.set_font('Arial', 'I', 10)
                    self.cell(30, 10, 'Tu Informacion de Salud Siempre Contigo', 0, 0, 'C')
                    self.ln(20)

            pdf = PDF()
            pdf.add_page()
            # Ficha de Paciente en PDF
            pdf.set_font("Arial", 'B', 12); pdf.set_fill_color(240, 240, 240)
            pdf.cell(0, 10, f"DATOS DEL PACIENTE", 1, 1, 'L', 1)
            pdf.set_font("Arial", '', 10)
            pdf.cell(0, 8, f"Nombre: {p.get('NOMBRE')}", 0, 1)
            pdf.cell(0, 8, f"Documento: {p.get('DOCUMENTO')} | RH: {p.get('RH')} | Edad: {p.get('EDAD')}", 0, 1)
            pdf.cell(0, 8, f"EPS: {p.get('EPS')}", 0, 1)
            pdf.set_text_color(200, 0, 0)
            pdf.multi_cell(0, 8, f"ALERTAS: {p.get('CONDICIONES ESPECIALES (ALERGIAS, ENFERMEDADES DE BASE)')}")
            pdf.set_text_color(0, 0, 0)
            pdf.cell(0, 8, f"EMERGENCIA: {p.get('NOMBRE CONTACTO EMERGENCIA')} ({p.get('TELEFONO CONTACTO EMERGENCIA')})", 0, 1)
            pdf.ln(10)

            # Evoluciones en PDF
            if not h_p.empty:
                pdf.set_font("Arial", 'B', 12); pdf.cell(0, 10, f"HISTORIAL DE EVOLUCIONES", 1, 1, 'L', 1)
                for _, f in h_p.iterrows():
                    ...
                    pdf.cell(0, 5, f"Talla: {f.get('4. TALLA')} | Peso: {f.get('5. PESO')} | Tension: {f.get('6. PRESIÓN ARTERIAL')}", 0, 1)
                    pdf.multi_cell(0, 5, f"Tratamiento: {f.get('8. MEDICAMENTOS')}")
                    pdf.ln(3); pdf.cell(0, 0, "", 'T', 1) # Línea divisoria
            
            st.download_button("📥 Descargar HC Completa (PDF)", pdf.output(dest='S').encode('latin-1'), f"HC_{id_buscado}.pdf")

            # Formulario Nueva Evolución
            with st.expander("➕ REGISTRAR NUEVA EVOLUCIÓN"):
                with st.form("f_evo", clear_on_submit=True):
                    ce1, ce2 = st.columns(2)
                    with ce1: v_val = st.text_area("Valoración"); v_mot = st.text_area("Motivo"); v_tal = st.text_input("Talla (cm)")
                    with ce2: v_pes = st.text_input("Peso (kg)"); v_pre = st.text_input("Presión"); v_med = st.text_area("Fórmula"); v_epi = st.text_area("Notas")
            ...
            st.subheader("📋 Historial en Pantalla")
            if not h_p.empty:
                for _, f in h_p.iterrows():
                    st.markdown(f"""
                    <div class="evo-card">
