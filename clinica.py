...
# --- 2. DISEÑO EN TONOS PASTEL (CON ALTA LEGIBILIDAD) ---
st.markdown("""
    <style>
    /* Fondo general - Menta Pastel Suave */
    .stApp {
        background-color: #f0fff4 !important;
    }

    /* FORZAR COLOR DE TEXTO EN CASILLAS (Negro para que se vea lo que escribes) */
    input, textarea, [data-baseweb="select"] {
        color: #000000 !important;
        background-color: #ffffff !important;
        border: 1.5px solid #a2d2ff !important; /* Borde azul pastel */
    }

    /* ETIQUETAS Y TÍTULOS - Azul Profundo para contraste */
    ...
    [data-testid="stSidebar"] label, [data-testid="stSidebar"] p {
        color: #581c87 !important; /* Texto morado oscuro */
    }
    ...
    /* TARJETAS DE HISTORIAL - Blanco con borde pastel */
    .medical-card {
...
if choice == "Registrar Paciente":
    ...
    with st.form("registro_completo", clear_on_submit=True):
        ...
        e_nombre = c1.text_input("Nombre de contacto")
        e_tel = c2.text_input("Teléfono de contacto")
...
# --- SECCIÓN 2: CONSULTA E HISTORIAL ---
    ...
    if id_buscar and df_pacientes is not None:
        ...
        if not p_data.empty:
            ...
            # MOSTRAR LO QUE YA EXISTE EN EL HISTORIAL
            ...
            if df_historial is not None:
                ...
                if not h_previo.empty:
                    st.dataframe(h_previo.iloc[::-1], use_container_width=True)
                else:
                    st.info("No se encontraron evoluciones previas para este paciente.")
            
            # AGREGAR NUEVA EVOLUCIÓN
            st.write("---")
            st.subheader("✍️ Registrar Nueva Evolución")
            with st.form("nueva_ev", clear_on_submit=True):
                trat = st.text_input("Tratamiento / Motivo")
                obs = st.text_area("Observaciones y Medicamentos")
