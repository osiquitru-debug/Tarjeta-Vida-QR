...
st.markdown(f"""
    ...
    /* FORMATO ORIGINAL: TARJETA DE PACIENTE */
    .medical-card {{
        background-color: #ffffff; padding: 20px; border-radius: 15px;
        border-left: 10px solid #a2d2ff; box-shadow: 0 4px 6px rgba(0,0,0,0.05);
        margin-bottom: 20px; color: #000000; text-align: left;
    }}
    
    /* FORMATO ORIGINAL: ALERTA EMERGENCIA */
    .emergency-box {{
    ...
    /* FORMATO ORIGINAL: TARJETA DE EVOLUCIÓN */
    .evo-card {{
        background-color: #ffffff; padding: 15px; border-radius: 10px;
        border: 1px solid #e2e8f0; border-left: 5px solid #b7e4c7;
        margin-bottom: 10px; color: #000000; text-align: left;
    }}

    /* CUADRÍCULA DE MEDIDAS ORIGINAL */
    .grid-medidas {{ 
        display: grid; grid-template-columns: 1fr 1fr 1fr; 
        gap: 10px; margin: 10px 0; font-size: 0.9em; 
    }}
...
if st.session_state.menu == "Inicio":
    ...
    if id_buscado and df_p is not None:
        ...
        if not paciente.empty:
            p = paciente.iloc[0]
            # TARJETA DE PACIENTE (FORMATO ORIGINAL RESTAURADO)
            st.markdown(f"""
            ...
            with st.expander("➕ REGISTRAR NUEVA EVOLUCIÓN"):
                with st.form("f_evo", clear_on_submit=True):
                    ...
                    with col_e1:
                        v_val = st.text_area("Valoración Física"); v_mot = st.text_area("Motivo"); v_tal = st.text_input("Talla (cm)")
                    with col_e2:
                        v_pes = st.text_input("Peso (kg)"); v_pre = st.text_input("Presión Arterial"); v_med = st.text_area("Fórmula"); v_epi = st.text_area("Notas")
            ...
            # HISTORIAL EN TARJETAS DE EVOLUCIÓN (FORMATO ORIGINAL RESTAURADO)
