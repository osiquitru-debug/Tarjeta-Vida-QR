...
# Cambios de color de fondo según la sección
...
if st.session_state.menu == "Inicio":
    ...
    with st.form("f_registro", clear_on_submit=True):
        ...
        with c1_e: c_nom = st.text_input("Nombre Referencia")
        with c2_e: c_tel = st.text_input("Teléfono Referencia")
    ...
    if id_buscado and df_p is not None:
        ...
        if not paciente.empty:
            ...
            pdf.set_font("Arial", 'B', 12); pdf.cell(0, 10, "HISTORIA CLINICA DEL PACIENTE", 1, 1, 'C')
            pdf.set_font("Arial", '', 10)
            pdf.cell(0, 7, f"Nombre: {p.get('NOMBRE')}", 0, 1)
            pdf.cell(0, 7, f"Doc: {p.get('DOCUMENTO')} | RH: {p.get('RH')} | Edad: {p.get('EDAD')}", 0, 1)
            pdf.multi_cell(0, 7, f"Alertas: {p.get('CONDICIONES ESPECIALES (ALERGIAS, ENFERMEDADES DE BASE)')}")
            pdf.ln(5)
            ...
            if not h_p.empty:
                for _, f in h_p.iterrows():
                    ...
                    pdf.cell(0, 5, f"SIGNOS: Talla: {f.get('4. TALLA')} | Peso: {f.get('5. PESO')} | Tension: {f.get('6. PRESIÓN ARTERIAL')}", 0, 1)
                    pdf.multi_cell(0, 5, f"TRATAMIENTO: {f.get('8. MEDICAMENTOS')}")
                    pdf.multi_cell(0, 5, f"EPICRISIS: {f.get('9. EPICRISIS O NOTAS ADICIONALES')}")
                    pdf.ln(2); pdf.cell(0, 0, "", 'T', 1); pdf.ln(2)

            st.download_button("📥 Descargar PDF HC", pdf.output(dest='S').encode('latin-1'), f"HC_{id_buscado}.pdf")

            # Formulario Evolución
            with st.expander("➕ NUEVA EVOLUCIÓN"):
                with st.form("f_evo", clear_on_submit=True):
                    ce1, ce2 = st.columns(2)
                    with ce1: v_val = st.text_area("Valoración"); v_mot = st.text_area("Motivo"); v_tal = st.text_input("Talla")
                    with ce2: v_pes = st.text_input("Peso"); v_pre = st.text_input("Presión"); v_med = st.text_area("Tratamiento"); v_epi = st.text_area("Epicrisis")
                    if st.form_submit_button("GUARDAR"):
                        data_e = {"entry.2019369477": id_buscado, "entry.1088523869": v_val, "entry.611862537": v_mot, "entry.1275746503": v_tal, "entry.949747647": v_pes, "entry.2091389798": v_pre, "entry.2016051626": v_med, "entry.616774918": v_epi}
            ...
            # HISTORIAL EN PANTALLA (CON TÍTULOS DE SHEET)
