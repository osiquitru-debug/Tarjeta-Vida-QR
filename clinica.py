...
# --- 1. CONFIGURACIÓN Y ESTILOS (FIDELIDAD VISUAL) ---
...
st.markdown(f"""
    ...
    /* CELDAS DE EVOLUCIÓN (BLANCO PURO Y LETRA NEGRA) */
    .evo-card {{
    ...
    .evo-card p, .evo-card b, .evo-card span {{
...
# --- 5. VISTA REGISTRAR (CONTACTO SEPARADO) ---
    ...
    with st.form("f_registro"):
        ...
        st.markdown("---")
        st.subheader("🚨 Contacto de Emergencia (Independiente)")
        col_sos1, col_sos2 = st.columns(2)
        with col_sos1:
            e_nombre = st.text_input("Nombre del Contacto de Emergencia")
        with col_sos2:
        ...
        if st.form_submit_button("GUARDAR DATOS"):
            ...
            st.success("✅ Registro guardado exitosamente."); st.cache_data.clear()
...
# --- 6. VISTA CONSULTA Y CARNET ---
    ...
    if id_buscado and df_p is not None:
        ...
        if not paciente.empty:
            ...
            q_file = f"temp_qr_{id_buscado}.png"
            qr_s.save(q_file, scale=10)
            ...
            if not h_p.empty:
                ...
                pdf_h.cell(0, 10, f"HISTORIAL: {p.get('NOMBRE')}", 0, 1, 'C')
                pdf_h.set_font("Arial", '', 10)
                pdf_h.cell(0, 7, f"ID: {p.get('DOCUMENTO')} | RH: {p.get('RH')} | SOS: {nom_e} ({tel_e})", 0, 1, 'L')
                pdf_h.ln(5)
                ...
                for _, f in h_p.iterrows():
                    ...
                    <div class="evo-card">
                        ...
                        <p style="margin:5px 0; color:#444;"><b>EPICRISIS:</b> {f.get('10. EPICRISIS')}</p>
                    </div>""", unsafe_allow_html=True)
