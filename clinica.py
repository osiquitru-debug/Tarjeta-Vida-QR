...
# Enlace directo desde PostIMG (estable)
...
if st.session_state.menu == "Inicio":
    ...
    st.info("Bienvenido al sistema de gestión médica para la comunidad de Guadalupe, Huila.")
    ...
    with st.form("form_registro_paciente", clear_on_submit=True):
        ...
        if st.form_submit_button("GUARDAR PACIENTE"):
            ...
            try:
                ...
                st.success(f"✅ {nombre} ha sido registrado correctamente."); st.cache_data.clear()
            except: st.error("Error en la conexión con la base de datos.")

elif st.session_state.menu == "Consulta":
    st.image(LOGO_URL, width=120)
    st.title("🔍 CONSULTA Y EVOLUCIÓN")
    busqueda_raw = st.text_input("Documento de identidad del paciente").strip()
    ...
    if id_buscado and df_p is not None:
        ...
        if not paciente.empty:
            ...
            if not h_p.empty:
                ...
                st.download_button(label="📥 Descargar Historial Completo (PDF)", data=pdf.output(dest='S').encode('latin-1'), file_name=f"HC_{id_buscado}.pdf", mime="application/pdf")

            with st.expander("➕ REGISTRAR NUEVA EVOLUCIÓN"):
                with st.form("f_evo", clear_on_submit=True):
                    c1, c2 = st.columns(2)
                    with c1:
                        v2 = st.text_area("Valoración Física"); v3 = st.text_area("Motivo de Consulta")
                        v4 = st.text_input("Talla (cm)"); v5 = st.text_input("Peso (kg)")
                    with c2:
                        v6 = st.text_input("Presión Arterial"); v7 = st.text_area("Antecedentes")
                        v8 = st.text_area("Medicamentos formulados"); v10 = st.text_area("Epicrisis / Notas")
                    if st.form_submit_button("GUARDAR EVOLUCIÓN"):
                        ...
                        st.success("Evolución guardada con éxito."); st.cache_data.clear(); st.rerun()
            ...
            else: st.info("No se registran evoluciones previas para este paciente.")
        else: st.error(f"Paciente con documento {id_buscado} no encontrado.")
