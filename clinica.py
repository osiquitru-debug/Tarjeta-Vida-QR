...
if st.session_state.menu == "Inicio":
    ...
    st.title("🔍 CONSULTA")
    id_buscado = st.text_input("Documento").strip()

    if id_buscado and df_p is not None:
        paciente = df_p[df_p['ID_KEY'] == id_buscado]
        if not paciente.empty:
            p = paciente.iloc[0]
            # TARJETA DE PACIENTE (FORMATO ORIGINAL)
            st.markdown(f"""
            <div class="medical-card">
                <h2 style='margin:0;'>👤 {p.get('NOMBRE')}</h2>
                <p><b>{p.get('TIPO DE DOCUMENTO')}:</b> {p.get('DOCUMENTO')} | <b>RH:</b> {p.get('RH')}</p>
